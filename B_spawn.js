...
var auto_respawn = function (spawn) {
	if (Game.spawns[spawn].room.energyAvailable < 100) {
		return;
	}
	// first spawn
	// essentials
	if (spawn_check(spawn, "harvester", 0, 1)) {
		return;
	} else if (spawn_check(spawn, "builder", 0, 1)) {
		return;
	} else if (spawn_check(spawn, "runner", 0, 1)) {
		return;
	} else if (spawn_check(spawn, "repairer", 0, 1)) {
		return;
	}

	// spawn to num
	// essentials
	else if (spawn_check(spawn, "harvester", 0, Game.spawns[spawn].room.find(FIND_SOURCES).length)) {
		return;
	} else if (
		spawn_check(
			spawn,
			"harvester",
			1,
			Game.spawns[spawn].room.find(FIND_MY_STRUCTURES, {
				filter: {
	...
	else if (spawn_check(spawn, "ranger", 0, 1)) {
		return;
	}
	...
	// claimer
	else if (Memory.roomTarget != "false" && spawn_check(spawn, "ranger", 1, 1)) {
		return;
	} else if (
		Memory.roomTarget != "false" &&
		!(Game.rooms[Memory.roomTarget] && Game.rooms[Memory.roomTarget].controller.my) &&
		spawn_check(spawn, "claimer", 0, 1)
	) {
		return;
	} else if (Memory.roomTarget != "false" && spawn_check(spawn, "builder", 1, 3)) {
		return;
	}
