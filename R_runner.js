...
var withdrawAll = function (creep, targets) {
  if (targets && targets.store) {
    res = Object.keys(targets.store).filter(
      (x) => x != RESOURCE_ENERGY && targets.store[x] != 0
    );
    if (res.length) {
...
function transferStructureTarget(
  creep,
  type,
  res = RESOURCE_ENERGY,
  minCap = 0
...
var doTask = function (creep) {
  ...
  if (creep.memory.building) {
    res = _.filter(
      ...
      (res) => res != RESOURCE_ENERGY && creep.store[res] != 0
    );
    if (res.length) {
    ...
    if (creep.store[RESOURCE_ENERGY] != 0) {
      if (creep.memory.task == 1) {
        if (
          doTransfer(transferStructureTarget(creep, STRUCTURE_SPAWN), creep)
        ) {
          return;
        }
        if (
          doTransfer(transferStructureTarget(creep, STRUCTURE_EXTENSION), creep)
        ) {
          return;
        }
        if (
          doTransfer(transferStructureTarget(creep, STRUCTURE_TOWER), creep)
        ) {
          return;
        }
        if (
          doTransfer(transferStructureTarget(creep, STRUCTURE_TERMINAL), creep)
        ) {
          return;
        }
  ...
  if (creep.memory.task == 1 && !res.length > 0) {
    targets = creep.pos.findClosestByRange(FIND_STRUCTURES, {
      filter: (targets) =>
        ...
        targets.store &&
        (targets.store.getUsedCapacity() >
          targets.store.getUsedCapacity(RESOURCE_ENERGY) ||
          (targets.store.getUsedCapacity(RESOURCE_ENERGY) == null &&
            targets.store.getUsedCapacity() > 0)),
    });
    if (targets) {
      res = _.filter(
        Object.keys(targets.store),
        (res) => res != RESOURCE_ENERGY && targets.store[res] != 0
      );
  ...
  targets = creep.pos.findClosestByRange(FIND_STRUCTURES, {
    filter: (targets) => {
      return (
        targets.structureType == STRUCTURE_CONTAINER &&
